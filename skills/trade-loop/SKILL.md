...
- Call **`get_log(limit=…)`** (the MCP tool — not Bash) and present a compact per-asset summary of recent ticks: last decision, strategies pass count, any open position + PnL, and the latest `iteration_id`/timestamp. Pull live position PnL from `get_account_state()`.
